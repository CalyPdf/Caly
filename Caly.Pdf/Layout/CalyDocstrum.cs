...
namespace Caly.Pdf.Layout
{
    ...
    public sealed class CalyDocstrum
    {
        ...
        {
            var withinLineDistList = new ConcurrentQueue<float>();
            var betweenLineDistList = new ConcurrentQueue<float>();

            // 1. Estimate within line and between line spacing
            CalyKdTree<PdfWord> calyKdTreeBottomLeft = new CalyKdTree<PdfWord>(words, w => w.BoundingBox.BottomLeft);
            ...
            Parallel.ForEach(words, parallelOptions, word =>
            {
                ...
                foreach (var n in calyKdTreeBottomLeft.FindNearestNeighbours(word, 2, w => w.BoundingBox.TopLeft, CalyDistances.Euclidean))
                {
                    ...
                    if (blBounds.Contains(angle))
                    {
                        ...
                        float dist = MathF.Abs(hypotenuse * MathF.Cos((90 - angle) * MathF.PI / 180))
        ...
        private static float? GetPeakAverageDistance(IReadOnlyCollection<float> distances, int binLength = 1)
        {
            ...
            float maxDbl = MathF.Ceiling(distances.Max());
            ...
            var bins = Enumerable.Range(0, (int)Math.Ceiling(max / (float)binLength) + 1)
                .Select(x => x * binLength)
                .ToDictionary(x => x, _ => new List<float>());
