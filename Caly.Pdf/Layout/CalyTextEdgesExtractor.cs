...
namespace Caly.Pdf.Layout
{
    ...
    /// </summary>
    public static class CalyTextEdgesExtractor
    {
        /// <summary>
        ...
        public static IReadOnlyDictionary<EdgeType, List<PdfLine>> GetEdges(IEnumerable<PdfWord> pageWords, int minimumElements = 4,
            int maxDegreeOfParallelism = -1)
        {
            ...
            var cleanWords = pageWords.Where(x => !x.Value.Trim().Span.IsWhiteSpace());

            ConcurrentDictionary<EdgeType, List<PdfLine>> dictionary = new ConcurrentDictionary<EdgeType, List<PdfLine>>();

            ParallelOptions parallelOptions = new ParallelOptions() { MaxDegreeOfParallelism = 1 }; // maxDegreeOfParallelism };
        ...
        private static List<PdfLine> GetVerticalEdges(IEnumerable<PdfWord> pageWords, Func<PdfRectangle, double> func, int minimumElements)
        {
            Dictionary<double, List<PdfWord>> edges = pageWords.GroupBy(x => func(x.BoundingBox))
                .Where(x => x.Count() >= minimumElements).ToDictionary(gdc => gdc.Key, gdc => gdc.ToList());

            Dictionary<double, List<List<PdfWord>>> cleanEdges = new Dictionary<double, List<List<PdfWord>>>();

            foreach (var edge in edges)
            {
                var sortedEdges = edge.Value.OrderBy(x => x.BoundingBox.Bottom).ToList();
                cleanEdges.Add(edge.Key, new List<List<PdfWord>>());
